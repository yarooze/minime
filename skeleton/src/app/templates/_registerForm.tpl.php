<div class="well" style="max-width: 400px; margin: 50px auto 10px;">
    <form role="form" method="post" id="<?php echo $form->getName(); ?>" name="<?php echo $form->getName(); ?>"
          action="<?php echo $app->router->getUrl('registerSubmit'); ?>">
      <div class="row notice">
        ...
        <?php endif;?>
          <div class="form-group">
            <input type="text" class="form-control-notice" id="login" name="form_register[login]" placeholder="Login" required
            pattern="[a-zA-Z0-9\._-]{3,25}" value="<?php $view->printString($form->getValue('login')); ?>"
            <?php echo (isset($errs['login'])) ? 'style="border:#FF0000 3px solid;"' : ''; ?> />
          </div>
          <?php if(isset($errs['login'])): ?>
            <p><i class="fa fa-exclamation-triangle"></i><strong>Error: </strong><?php $view->printString($errs['login']); ?></p>
          <?php endif; ?>
          <div class="form-group">
    ...
    <script type="text/javascript">
      window.onload = function () {
        ...
        document.getElementById("login").oninvalid = invalidLogin;
      }
      ...
      function invalidLogin(e) {
        ...
        if (!e.target.validity.valid) {
          e.target.setCustomValidity("<?php $view->printString($form->getErrorMsg('login')); ?>");
        }
      }
    </script>
</div>