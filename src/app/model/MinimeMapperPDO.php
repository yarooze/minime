 ...
 *
 */
abstract class MinimeMapperPDO extends MinimeMapper
{
    /**
    ...
    protected function getIdAsIdQueryPart() {        
        ...
        if (is_string($this->id_field)) {
            return ' ' . $this->alias . '.' . $this->id_field . ' AS id, ';
        }        
        
        $id_as_id = '';
        foreach($this->id_field as $id_field) {            
            //$id_as_id = ' ' . $this->alias . '.' . $id_field . ' AS '.$id_field.', ';
        }
        return $id_as_id;
    }
    ...
    protected function buildQueryConditions($filter, &$q_where, &$args, $glue = 'AND') {
        ...
        foreach ($filter as $r_name => $r_value) {
            if (in_array($r_name, array('AND', 'OR'), true)) {
                ...
                $dbfield = $r_name;
                $mapping = $this->getMappingByFieldName($r_name, $entity);
    ...
    public function retrieveCollection(&$params = array(), $asEntites = false)
    {
        ...
        if (!isset($order)) {
            ...
            if (!is_array($id_fields)) {
                $id_fields = array($id_fields);
            }
            foreach ($id_fields as $id_field) {
                $order = array(array('name' => '' . $id_field, 'order' => 'DESC'));
            }
        }
...
//    echo "</pre>";
        ...
        if ($asEntites && !empty($rows)) {
            $result = array();
            foreach($rows as $row) {
                $entity = $this->createEntity();
                $this->setFieldsFromArray($row, $entity);
                $result[] = $entity;
            }
            $rows = $result;
        }
     ...
     */
    public function retrieveById($id, $asEntity = false)
    {
        $data = null;
     ...
     * @return Ambigous <NULL, mixed>
     */
    public function retrieveOneBy($field, $value, $asEntity = false)
    {
        $data = null;
        $q = 'SELECT
          ' . $this->getIdAsIdQueryPart() . '
